...
CLUSTER_NAME="${CLUSTER_NAME:-gargantua}"
...
BOOTSTRAP_APP_FILE="$ROOT_DIR/KUBE/clusters/gargantua/core.yaml"
...
- ENDR UI/API (gateway): https://portal.k8s.local, https://api.k8s.local
...
- 127.0.0.1 argocd.k8s.local portal.k8s.local api.k8s.local mann.k8s.local miller.k8s.local edmund.k8s.local
EOF_SUMMARY
}
