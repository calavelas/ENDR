...
def _read_yaml_dict(path: str) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(
            "PyYAML is required for config loading. Install dependencies in ENDR first."
        ) from exc
...
def build_validation_report() -> ConfigValidationReport:
    ...
    try:
        ...
        errors.extend(_format_pydantic_error("ENDR", exc))
    except Exception as exc:  # noqa: BLE001
        errors.append(f"ENDR: {exc}")

    try:
        services_config = load_services_config(paths.servicesConfigPath)
    except ValidationError as exc:
        errors.extend(_format_pydantic_error("services", exc))
    except Exception as exc:  # noqa: BLE001
        errors.append(f"SVCS: {exc}")
