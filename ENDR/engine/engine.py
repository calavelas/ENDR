...
def update_image_tags_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Update image tags in services.yaml for selected services")
    parser.add_argument("--repo-root", default=".")
    ...
    idp_path = repo_root / "platform.yaml"
    ...
    if not isinstance(idp_raw, dict):
        raise ValueError("platform.yaml root must be a mapping")
    owner = str(idp_raw.get("config", {}).get("git", {}).get("owner", "")).strip()
    if not owner:
        raise ValueError("platform.yaml config.git.owner is required")
    ...
    for service_name in services:
        ...
        if not isinstance(overrides, dict):
            raise ValueError(f"services.yaml overrides for service '{service_name}' must be mapping")
