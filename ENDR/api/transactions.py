...
def _select_pipeline_state(
    ...
    if not pull_request.merged:
        ...
        if str(pr_check_run.get("status") or "") != "completed":
            notifications.append("Reconcile PR check is running.")
    ...
    if not reconcile_run:
        return ("pending", "Waiting for Reconcile workflow to start.", notifications)

    if str(reconcile_run.get("status") or "") != "completed":
        notifications.append("Reconcile is running.")
        return ("running", "Reconcile is running.", notifications)

    reconcile_conclusion = str(reconcile_run.get("conclusion") or "")
    if reconcile_conclusion != "success":
        notifications.append(f"Reconcile ended with {reconcile_conclusion or 'unknown'} state.")
        return ("failed", f"Reconcile failed ({reconcile_conclusion or 'unknown'}).", notifications)

    if not svcs_run:
        return ("pending", "Waiting for Service build/deploy workflow to start.", notifications)

    if str(svcs_run.get("status") or "") != "completed":
        notifications.append("Service build/deploy is running.")
        return ("running", "Service build/deploy is running.", notifications)

    svcs_conclusion = str(svcs_run.get("conclusion") or "")
    if svcs_conclusion == "success":
        notifications.append("Service build/deploy finished successfully.")
        return ("success", "Pipeline finished successfully.", notifications)

    notifications.append(f"Service build/deploy ended with {svcs_conclusion or 'unknown'} state.")
    return ("failed", f"Service build/deploy failed ({svcs_conclusion or 'unknown'}).", notifications)
...
def _timeline_for_status(status: TransactionStatusResponse, submitted_at: str | None) -> list[TransactionTimelineEvent]:
    ...
    if submitted_at:
        events.append(
            TransactionTimelineEvent(
                id="case-submit",
                title="Submitted from portal",
                status="success",
                timestamp=submitted_at,
                detail="Portal created the branch and pull request.",
    ...
    events.append(
        TransactionTimelineEvent(
            ...
            detail="Pull request opened from the portal.",
            url=status.pullRequest.htmlUrl or None,
        )
    )
    ...
    if not status.pullRequest.merged:
        ...
        events.append(
            TransactionTimelineEvent(
                ...
                title="Reconcile",
                status="pending",
                detail="Waiting for Reconcile workflow.",
            )
        )
        ...
        events.append(
            TransactionTimelineEvent(
                ...
                detail="Reconcile workflow is running.",
        ...
        events.append(
            TransactionTimelineEvent(
                ...
                status="success",
                timestamp=reconcile_run.updatedAt or reconcile_run.createdAt,
                detail="Reconcile workflow completed successfully.",
                url=reconcile_run.htmlUrl or None,
            )
        )
    else:
        events.append(
            TransactionTimelineEvent(
                ...
                status="failed",
                timestamp=reconcile_run.updatedAt or reconcile_run.createdAt,
                detail=f"Reconcile failed ({reconcile_run.conclusion or 'unknown'}).",
                url=reconcile_run.htmlUrl or None,
            )
        )
    ...
    if not status.pullRequest.merged:
        ...
        events.append(
            TransactionTimelineEvent(
                ...
                title="Build and Deploy",
                status="pending",
                detail="Waiting for Service build/deploy workflow.",
            )
        )
        ...
        events.append(
            TransactionTimelineEvent(
                ...
                detail="Service build/deploy workflow is running.",
        ...
        events.append(
            TransactionTimelineEvent(
                ...
                status="success",
                timestamp=svcs_run.updatedAt or svcs_run.createdAt,
                detail="Service build/deploy workflow completed successfully.",
                url=svcs_run.htmlUrl or None,
            )
        )
    else:
        events.append(
            TransactionTimelineEvent(
                ...
                status="failed",
                timestamp=svcs_run.updatedAt or svcs_run.createdAt,
                detail=f"Service build/deploy failed ({svcs_run.conclusion or 'unknown'}).",
                url=svcs_run.htmlUrl or None,
            )
        )
...
def _build_status_from_github(
    ...
    pr_check_run = _select_run_after(pr_runs, "reconcile-pr.yml", float("-inf"))
    ...
    if pull.mergeCommitSha:
        ...
        reconcile_run = _select_run_after(main_runs, "reconcile.yml", merged_threshold_ms)
    ...
    if reconcile_run:
        ...
        if math.isfinite(reconcile_created_ms):
            # Service build/deploy should start after current reconcile/update run starts.
        ...
        if not svcs_run:
            svcs_run = _select_run_after(main_runs, "services-build.yml", svcs_threshold_ms)
    ...
    *,
    ...
    placeholder = TransactionStatusResponse(
        pullRequest=TransactionPullRequest(
            ...
            title=f"{PORTAL_PR_TITLE_PREFIX} {safe_service_name}",
    ...
    if isinstance(existing, dict):
        ...
        if cached_status:
            ...
            if math.isfinite(now_ms) and math.isfinite(last_synced_ms):
                open_cache_ms = _env_int("API_TXN_STATUS_OPEN_CACHE_MS", _TXN_STATUS_OPEN_CACHE_MS)
                closed_cache_ms = _env_int("API_TXN_STATUS_CLOSED_CACHE_MS", _TXN_STATUS_CLOSED_CACHE_MS)
...
def _history_from_state(
    *,
    ...
    return CaseHistoryResponse(
        ...
        titlePrefix=PORTAL_PR_TITLE_PREFIX,
...
def get_case_history(
    *,
    ...
    max_refresh_candidates = _env_int("API_HISTORY_REFRESH_MAX_CANDIDATES", _HISTORY_REFRESH_MAX_CANDIDATES)
    stale_threshold_ms = _env_int("API_HISTORY_REFRESH_STALE_MS", _HISTORY_REFRESH_STALE_MS)
    if safe_limit >= 100:
