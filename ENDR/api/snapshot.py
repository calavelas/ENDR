...
def _normalize_svcs_config_url(url: str) -> str:
    value = url.strip()
    if not value:
        return DEFAULT_SERVICES_CONFIG_RAW_URL
...
def _resolve_svcs_config_url() -> str:
    configured = os.getenv("API_SERVICES_CONFIG_URL", DEFAULT_SERVICES_CONFIG_BLOB_URL)
...
def _fetch_remote_services_config(url: str) -> ServicesConfig:
    ...
    if parsed.scheme == "https":
        verify_tls = os.getenv("API_SERVICES_VERIFY_TLS", "true").strip().lower() not in {"0", "false", "no"}
        if verify_tls:
            try:
                import certifi
    ...
    if not raw.strip():
        raise ValueError(f"empty services.yaml payload from {url}")

    try:
        import yaml
    except ModuleNotFoundError as exc:
        raise RuntimeError("PyYAML is required for services.yaml parsing") from exc

    parsed = yaml.safe_load(raw) or {}
    if not isinstance(parsed, dict):
        raise ValueError(f"services.yaml root must be a mapping in {url}")
    return ServicesConfig(**parsed)


def load_platform_configs() -> tuple[IDPConfig, ServicesConfig, Any, str]:
...
def _build_service_url(service_name: str) -> str:
    scheme = os.getenv("API_SERVICE_GATEWAY_SCHEME", DEFAULT_SERVICE_GATEWAY_SCHEME).strip() or DEFAULT_SERVICE_GATEWAY_SCHEME
    domain = os.getenv("API_SERVICE_GATEWAY_DOMAIN", DEFAULT_SERVICE_GATEWAY_DOMAIN).strip() or DEFAULT_SERVICE_GATEWAY_DOMAIN
...
def _build_config_snapshot() -> PlatformSnapshot:
    idp_config, services_config, paths, svcs_url = load_platform_configs()
    ...
    for index, service in enumerate(sorted(services_config.services, key=lambda item: item.name), start=1):
        services.append(
            ServiceNode(
                ...
                syncStatus="Unknown",
                healthStatus="Unknown",
                sourcePath=f"services/{service.name}/chart",
                revision="main",
                deployedAt=None,
...
def build_platform_snapshot() -> PlatformSnapshot:
    ...
    # Guard against stale in-memory expectations by re-reading services.yaml when
