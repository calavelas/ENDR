...
- `ENDR/backstage/`: Backstage alternative track (templates + custom scaffolder actions).
- `DOCS/`: architecture, setup, phase plans, and portfolio docs.
- `ENDR/api/`: backend portal domain module (`/api/plex`).
...
- `svcs-build.yml`: build/publish service images for changed service source under `services/<name>/` (excluding `chart/`), then persist image tag updates.
