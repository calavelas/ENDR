...
- `make -f ENDR/scripts/Makefile validate-config` to validate `ENDR.yaml` and `SVCS.yaml`.
- `make -f ENDR/scripts/Makefile svcs-check` to run Phase 1 reconcile check (dry-run, no PR).
- `make -f ENDR/scripts/Makefile svcs-sync` to render reconcile changes into working tree.
...
## CI/CD Notes
- Workflow `tars-init.yml` runs on PR changes to `ENDR.yaml`/`SVCS.yaml`, auto-tags changed service images (`git-<sha>`), executes `ENDR/engine/engine.py svcs-check --write-worktree`, and auto-commits generated assets back to the same PR branch.
...
- On merge to `main`, workflow `svcs-build.yml` detects changed source services, auto-sets `git-<sha>` image tags, builds images from `services/<name>/Dockerfile`, pushes to Docker Hub, and persists updated tags back to repo.
- Workflow `svcs-build.yml` also supports manual `workflow_dispatch` to publish all services in one run.
- Workflow `svcs-tagged.yml` runs on Git tags in format `servicename.Vx.x.x`, builds image from `services/servicename`, and publishes Docker tag `Vx.x.x`.
- On PR/main changes for ENDR frontend/backend paths, workflow `platform-build.yml` builds `plex` + `case`; PRs validate build only, and `main` pushes images then updates `ENDR/api/chart/values.yaml` and `ENDR/portal/chart/values.yaml` with the new short commit SHA tag (`<sha7>`).
